...
class HoneyPotServer(rdp.RDPServerObserver):
    ...
    def onReady(self):
        ...
        if self._rssFile is None:
            ...
            log.info("%s --- select file (%s, %s) -> %s"%(datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%S.%fZ'),width, height, rssFilePath))
            sys.stdout.write("%s --- select file (%s, %s) -> %s"%(datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%S.%fZ'),width, height, rssFilePath))
            self._rssFile = rss.createReader(rssFilePath)
        
        domain, username, password = self._controller.getCredentials()
        hostname = self._controller.getHostname()
        log.info("""%s --- Credentials: domain: %s username: %s password: %s hostname: %s"""%(datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%S.%fZ'), domain, username, password, hostname));
        sys.stdout.write("""%s --- Credentials: domain: %s username: %s password: %s hostname: %s"""%(datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%S.%fZ'), domain, username, password, hostname));
        self.start()
...
class HoneyPotServerFactory(rdp.ServerFactory):
    ...
    def buildObserver(self, controller, addr):
        ...
        """
        log.info("%s --- Connection from %s:%s"%(datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%S.%fZ'), addr.host, addr.port))
        sys.stdout.write("%s --- Connection from %s:%s"%(datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%S.%fZ'), addr.host, addr.port))
...
if __name__ == '__main__':
    ...
    #build size map
    log.info("%s --- Start rdphoneypot"%datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%S.%fZ'))
    sys.stdout.write("%s --- Start rdphoneypot"%datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%S.%fZ'))
    log.info("%s --- Build size map"%datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%S.%fZ'))
    sys.stdout.write("%s --- Start rdphoneypot"%datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%S.%fZ'))
    for arg in args:
        size = readSize(arg)
        rssFileSizeList.append((size, arg))
        log.info("%s --- (%s, %s) -> %s"%(datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%S.%fZ'), size[0], size[1], arg))
        sys.stdout.write("%s --- (%s, %s) -> %s"%(datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%S.%fZ'), size[0], size[1], arg))
